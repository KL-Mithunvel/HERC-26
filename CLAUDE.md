...
- Continuously logs every poll cycle to JSONL, and optionally to SQLite with session tracking
...
### `logger/sqlite_db.py` — SQLite Logger (Optional / Advanced)

- `SQLiteLogger(db_path)` class; not used in `main.py` by default.
...
- DB stored at `data/rover_logs.sqlite`.
...
32. **`SQLiteLogger` is optional and must not be a hard dependency.** `main.py` must run successfully without it. Do not add it to `main.py` as a required import.
